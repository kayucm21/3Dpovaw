#!/bin/bash
# ═══════════════════════════════════════════════════════════════
#  NaiveProxy Panel v7.0 — Ultra-Fast Installer
#  Установка за 60 секунд с полной VDS-оптимизацией
#  VLESS Reality + XTLS-Vision (максимальное шифрование)
...
header() {
  ...
  echo -e "${PURPLE}${BOLD}║     NaiveProxy Panel v7.0 — Ultra-Fast Installer             ║${RESET}"
  echo -e "${PURPLE}${BOLD}║     VLESS Reality + XTLS-Vision (MAX Security)               ║${RESET}"
  echo -e "${PURPLE}${BOLD}╚═══════════════════════════════════════════════════════════════╝${RESET}"
  echo ""
}
...
# ═══════════════════════════════════════════════════════════════
# VDS AUTO-DETECT & OPTIMIZE (enhanced)
# ═══════════════════════════════════════════════════════════════
...
cat > "$SYSCTL_CONF" <<EOF
# NaiveProxy Panel v7.0 — VDS Optimizations
...
log_ok "VDS оптимизирован: BBR + TCP tuning + limits + Reality ready"
...
# Проверка доступности домена для VLESS Reality
if [[ "$INSTALL_PROTOCOL" == "vless" ]]; then
  echo ""
  echo -e "${CYAN}${BOLD}🔍 Проверка домена для VLESS Reality...${RESET}"
  if ! dig +short "$NAIVE_DOMAIN" >/dev/null 2>&1; then
    echo -e "${YELLOW}⚠ Домен $NAIVE_DOMAIN не имеет DNS записей. Убедитесь что A-запись настроена.${RESET}"
    read -rp "  Продолжить? (y/N): " CONTINUE_INSTALL
    if [[ "$CONTINUE_INSTALL" != "y" && "$CONTINUE_INSTALL" != "Y" ]]; then
      echo "Установка отменена"
      exit 1
    fi
  else
    echo -e "${GREEN}✅ Домен доступен${RESET}"
  fi
fi
...
if [[ "$INSTALL_PROTOCOL" == "vless" ]]; then
  log_info "Протокол: VLESS + Reality + XTLS-Vision"
  log_info "SNI: Будет выбран случайный из списка (Cloudflare, Microsoft, Apple...)"
fi
echo ""
...
# Проверка что node_modules создан
if [[ ! -d "node_modules/express" ]]; then
  echo -e "${RED}${BOLD}❌ Ошибка: npm зависимости не установлены!${RESET}"
  echo -e "${YELLOW}Попробуйте вручную: cd $PANEL_DIR/panel && npm install${RESET}"
  exit 1
fi
...
if [[ "$INSTALL_PROTOCOL" == "naive" ]]; then
  ...
  # Для VLESS покажем заглушку — реальная ссылка будет после установки Reality
  echo -e "${BOLD}🔗 VLESS Reality (после установки):${RESET}"
  echo -e "   ${CYAN}vless://${VLESS_UUID}@${NAIVE_DOMAIN}:443?encryption=none&security=reality&sni=xxx&fp=chrome&pbk=xxx&sid=xxx&type=tcp&flow=xtls-rprx-vision#Reality${RESET}"
  echo ""
  echo -e "${BOLD}📊 Параметры (будут показаны после установки):${RESET}"
  echo -e "   UUID:  ${CYAN}${VLESS_UUID}${RESET}"
  echo -e "   Port:  ${CYAN}443${RESET}"
  echo -e "   SNI:   ${CYAN}Auto (выбирается из списка популярных доменов)${RESET}"
  echo -e "   Flow:  ${CYAN}xtls-rprx-vision${RESET}"
  echo ""
