#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════
#  Панель NaiveProxy — Полный установщик
...
header() {
  # clear only when running in a real TTY
  if [[ -t 1 ]]; then
    clear || true
  fi
  echo ""
  echo -e "${PURPLE}${BOLD}╔══════════════════════════════════════════════════════════╗${RESET}"
  echo -e "${PURPLE}${BOLD}║          Панель NaiveProxy — Установщик                 ║${RESET}"
  echo -e "${PURPLE}${BOLD}╚══════════════════════════════════════════════════════════╝${RESET}"
  echo ""
}
...
print_access_option "1" "Через Nginx на порту ${BOLD}8080${RESET} ${GREEN}(рекомендуется — порт 3000 не светится)${RESET}"
print_access_option "2" "Напрямую на порту ${BOLD}3000${RESET} (проще, но порт виден)"
print_access_option "3" "Через Nginx с доменом + HTTPS (максимальная защита)"
echo ""
ACCESS_MODE_FROM_PROMPT="0"
if [[ -z "${ACCESS_MODE:-}" ]]; then
  if [[ -t 0 ]]; then
    read -rp "Ваш выбор [1/2/3]: " ACCESS_MODE
    ACCESS_MODE_FROM_PROMPT="1"
  else
    log_warn "Запуск без интерактива (stdin не TTY) — выбираем по умолчанию: 1"
    ACCESS_MODE="1"
  fi
fi
ACCESS_MODE="${ACCESS_MODE:-1}"

# Reprint menu with ✅ after selection when chosen via prompt
if [[ "$ACCESS_MODE_FROM_PROMPT" == "1" ]]; then
  echo ""
  print_access_option "1" "Через Nginx на порту ${BOLD}8080${RESET} ${GREEN}(рекомендуется — порт 3000 не светится)${RESET}"
  print_access_option "2" "Напрямую на порту ${BOLD}3000${RESET} (проще, но порт виден)"
  print_access_option "3" "Через Nginx с доменом + HTTPS (максимальная защита)"
...
echo -e "${BOLD}Выберите протокол прокси:${RESET}"
echo ""
echo -e "  ${CYAN}1)${RESET} Naive (Caddy forwardproxy)"
echo -e "  ${CYAN}2)${RESET} VLESS (Xray + Caddy WS+TLS)"
echo ""
if [[ -t 0 ]]; then
  read -rp "Ваш выбор [1/2]: " PROTOCOL_CHOICE
else
  log_warn "Запуск без интерактива (stdin не TTY) — выбираем по умолчанию: 1 (Naive)"
  PROTOCOL_CHOICE="1"
fi
...
echo ""
read -rp "  Домен для прокси (например vpn.yourdomain.com): " NAIVE_DOMAIN
...
[[ -z "$VLESS_UUID" ]] && VLESS_UUID=$(openssl rand -hex 16 | sed 's/\(..\)/\1/g' | head -c 36)
VLESS_PORT="443"
VLESS_WS_PATH="/vless"
...
if [[ "$INSTALL_PROTOCOL" == "naive" ]]; then
  ...
  log_info "Порт:   ${VLESS_PORT}"
  log_info "Path:   ${VLESS_WS_PATH}"
fi
echo ""
...
# ── Установка выбранного протокола прокси ───────────────────────────────
if [[ "$INSTALL_PROTOCOL" == "vless" ]]; then
  log_step "Установка VLESS (Xray + Caddy)..."
  ...
  bash "${PANEL_DIR}/panel/scripts/install_vless.sh" || {
    log_err "Установка VLESS не удалась"
    exit 1
  }
...
if [[ ! -f "${PANEL_DIR}/panel/data/config.json" ]]; then
  ...
  if [[ "$INSTALL_PROTOCOL" == "vless" ]]; then
    FIRST_PASSWORD="${VLESS_UUID}"
  else
    FIRST_PASSWORD="${NAIVE_PASS}"
  fi
...
# ── Автообновление панели раз в сутки (systemd timer) ───────────────────
log_step "Настройка автообновления панели (раз в сутки)..."
cat > /etc/systemd/system/naiveproxy-panel-autoupdate.service <<'EOF'
[Unit]
Description=NaiveProxy panel auto-update
After=network-online.target
Wants=network-online.target

[Service]
Type=oneshot
ExecStart=/bin/bash /opt/naiveproxy-panel/panel/scripts/panel_auto_update.sh
EOF

cat > /etc/systemd/system/naiveproxy-panel-autoupdate.timer <<'EOF'
[Unit]
Description=Daily NaiveProxy panel auto-update
...
systemctl enable --now naiveproxy-panel-autoupdate.timer >/dev/null 2>&1 || true
log_ok "Автообновление включено (systemd timer: naiveproxy-panel-autoupdate.timer)"
...
VLESS_LINK="vless://${VLESS_UUID}@${NAIVE_DOMAIN}:${VLESS_PORT}?encryption=none&security=tls&type=ws&host=${NAIVE_DOMAIN}&sni=${NAIVE_DOMAIN}&path=%2Fvless#${NAIVE_LOGIN}"

echo ""
echo -e "${PURPLE}${BOLD}╔══════════════════════════════════════════════════════════════╗${RESET}"
echo -e "${PURPLE}${BOLD}║                                                              ║${RESET}"
echo -e "${PURPLE}${BOLD}║   ✅  Панель NaiveProxy — Установка завершена!               ║${RESET}"
echo -e "${PURPLE}${BOLD}║                                                              ║${RESET}"
echo -e "${PURPLE}${BOLD}╠══════════════════════════════════════════════════════════════╣${RESET}"
echo -e "${PURPLE}${BOLD}║                                                              ║${RESET}"
