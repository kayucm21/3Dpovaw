...
if [[ -f "$REALITY_KEYS_FILE" ]]; then
  ...
  REALITY_SNI=$(jq -r '.serverNames[1] // ""' "$REALITY_KEYS_FILE" 2>/dev/null || echo "")
fi
...
if [[ "$PROTOCOL" == "vless" ]]; then
  ...
  if [[ -f "$PANEL_DIR/panel/data/vless-uuid.txt" ]]; then
    ...
    # Reality info
    if [[ -n "$REALITY_PUBLIC" && -n "$REALITY_SHORTID" ]]; then
      echo -e "   🔒 Security: ${CYAN}Reality + XTLS-Vision${RESET}"
      echo -e "   SNI:        ${CYAN}${REALITY_SNI:-auto}${RESET}"
      echo -e "   Public Key: ${CYAN}${REALITY_PUBLIC:0:16}...${RESET}"
      echo -e "   Short ID:   ${CYAN}${REALITY_SHORTID}${RESET}"
      echo ""
      echo -e "${BOLD}🔗 VLESS Reality ссылка:${RESET}"
      echo -e "   ${GREEN}vless://${UUID}@${DOMAIN_PROXY}:${VLESS_PORT}?encryption=none&security=reality&sni=${REALITY_SNI:-${DOMAIN_PROXY}}&fp=chrome&pbk=${REALITY_PUBLIC}&sid=${REALITY_SHORTID}&type=tcp&flow=xtls-rprx-vision#Reality-VLESS${RESET}"
    else
      echo ""
